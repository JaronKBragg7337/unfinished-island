...
const archiveObjects=new Map();
function growArchive(){
 for(const c of worldState.children.slice(-16)){
  if(archiveObjects.has(c.id))continue;
  const index=worldState.children.indexOf(c),slot=index%16;
  // A physical record case is added for each published child; the history retains all older worlds.
  const g=register(new THREE.Group(),`island-0001/archive/${c.id}`,`World archive: ${c.name}`,true);g.userData.childId=c.id;
  g.position.set(6+Math.floor(slot/8)*1.25,1,-5+(slot%8)*1.3);island.add(g);
  cylinder(g,.26,.7,0,.35,0,mats.steel);box(g,.6,.06,.46,0,.74,0,mats.wood);
  box(g,.48,.38,.12,0,.98,0,mats.dark);textPlane(g,c.id,0,.97,.066,.43,.15);
  const glass=new THREE.Mesh(new THREE.SphereGeometry(.13,12,8),new THREE.MeshPhysicalMaterial({color:0x7ccab2,metalness:.1,roughness:.15,transparent:true,opacity:.75}));glass.position.set(0,1.35,0);g.add(glass);
  archiveObjects.set(c.id,g);
 }
 const retained=new Set(worldState.children.slice(-16).map(c=>c.id));for(const [id,o]of archiveObjects)if(!retained.has(id)){island.remove(o);archiveObjects.delete(id);}
}
async function refresh(){try{const next=await json('world.json');worldState=next;if(!activeChild&&next.children.length)await loadChild(next.children.at(-1));growArchive();status();}catch(e){$('#status span').textContent='Record unavailable · retrying';console.error(e);}}
