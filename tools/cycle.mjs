...
import {evaluateRevision} from '../src/revision.js';
...
try{
 ...
 let program,author,raw='',attempts=[],diagnostics;
 let prompt=`You are Mara, a resident at terminal island-0001/workstation-01. You are explicitly assigned to create a child simulation and improve food deposition. Write an executable declarative Island Language program, JSON only. No shell, JavaScript, markdown, or extra fields. Grammar: language="island/1", name:string max70, hypothesis:string max800, seed:integer 1..1000000, population:integer 2..12, speed:number 0.3..2, sense:number 2..16, food:3..20 pairs [x,z] within -18..18, rules:1..8 {when,do}. Conditions: always,carrying,tired,foodNear. Actions: gather,deposit,rest,explore. First matching rule wins. Carrying capacity 3; deposit moves home and stores food; tired is energy<28; gather only helps with nearby food; include always fallback. Motion costs 0.8 energy/metre, rest restores4. Patches regenerate1 each30 ticks. Objective: design and explain a foraging society, then test it. Prior results: ${JSON.stringify(state.children.slice(-3).map(c=>({name:c.name,results:c.results,hypothesis:c.hypothesis}))).slice(0,4500)}. Example syntax: ${JSON.stringify(starter)}. Create your own program.`;
 const parentRecord=process.argv.includes('--seed')?null:(state.children.find(c=>c.id===state.policyChampionId)||state.children.at(-1));
 const parentProgram=parentRecord?JSON.parse(await fs.readFile(path.join(data,parentRecord.source),'utf8')):null;
 if(parentRecord){
  state.policyChampionId=parentRecord.id;
  const priorEvidence=await Promise.all(state.children.filter(c=>c.revision).slice(-4).map(async c=>({id:c.id,rules:JSON.parse(await fs.readFile(path.join(data,c.source),'utf8')).rules,training:c.revision.training,adopted:c.revision.adopted,diagnostics:c.diagnostics||diagnose(JSON.parse(await fs.readFile(path.join(data,c.source),'utf8')))})));
  prompt=`You are Mara. You are explicitly assigned to improve the ordered foraging rules of your existing world ${parentRecord.id}. The environment is FIXED. Return JSON with EXACTLY three keys: name (1..70 characters), hypothesis (max800 characters), rules (1..8 objects {when,do}). Do not return environment fields. Allowed conditions: always,carrying,tired,foodNear. Actions: gather,deposit,rest,explore. First matching rule wins; include always. carrying means cargo>0 (capacity3); tired means energy<28. gather moves toward nearest nonempty patch and harvests; deposit returns home and stores cargo; rest restores4 energy per step; movement costs0.8 per metre. Score is deposited food after720 steps. Parent program (hypothesis is unverified text): ${JSON.stringify(parentProgram)}. Parent execution: ${JSON.stringify(diagnose(parentProgram))}. Previous revision evidence: ${JSON.stringify(priorEvidence)}. Propose a specific rule change based on evidence. Your proposed world remains in history even if it performs worse. Only measured improvement can replace the previous policy. A new name is not an improvement.`;
 }
 try{
  if(process.argv.includes('--seed')){program=starter;author='Codex / authored seed';}
  else{
   const proposal=await propose(parentProgram,prompt,async (request,format)=>{
   const response=await fetch('http://127.0.0.1:11434/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({model:'qwen3.5:4b',stream:false,think:false,format,messages:[{role:'user',content:request}],options:{temperature:0.8,num_predict:1800,num_ctx:8192}}),signal:AbortSignal.timeout(240000)});
   ...
   ({program,raw,attempts,diagnostics}=proposal);author='Mara / qwen3.5:4b';
  }
  ...
  const revision=parentProgram?{of:parentRecord.id,...evaluateRevision(program,parentProgram)}:null;
  const child={diagnostics,attemptCount:attempts.length,revision,id,parentId:'island-0001',terminalId:'island-0001/workstation-01',created:now,author,assignment:'Explicitly assigned child-world experiment',name:program.name,hypothesis:program.hypothesis,hash,source:`${id}.island.json`,snapshot:`${id}.snapshot.json`,results};
  if(revision){if(revision.adopted)state.policyChampionId=id;state.events.push({at:now,type:'policy-evaluated',id,message:`Revision of ${parentRecord.id}: ${revision.adopted?'adopted':'previous policy retained'}; training food delta ${revision.trainingDelta}, holdout delta ${revision.holdoutDelta}.`});}
  ...
  await write(path.join(data,`${id}.record.json`),{...child,prompt,raw:raw||source,attempts});
 }catch(error){state.events.push({at:now,type:'experiment-failed',id,message:String(error.message)});await write(path.join(data,`${id}.failure.json`),{at:now,error:String(error.message),raw,prompt,attempts:error.attempts||attempts});}
 ...
 execFileSync(process.execPath,['--test','tests/sim.test.mjs','tests/records.test.mjs','tests/revision.test.mjs','tests/diagnostics.test.mjs'],{stdio:'inherit',timeout:60000});
