...
import {revise,evaluateRevision} from '../src/revision.js';
...
try{
 ...
 try{
  if(process.argv.includes('--seed')){program=starter;author='Codex / authored seed';}
   ...
   const result=await response.json();raw=result.message.content;program=parentProgram?revise(parentProgram,JSON.parse(raw)):validate(JSON.parse(raw));author='Mara / qwen3.5:4b';
  }
  ...
  const child={revision,id,parentId:'island-0001',terminalId:'island-0001/workstation-01',created:now,author,assignment:'Explicitly assigned child-world experiment',name:program.name,hypothesis:program.hypothesis,hash,source:`${id}.island.json`,snapshot:`${id}.snapshot.json`,results};
 ...
 execFileSync(process.execPath,['--test','tests/sim.test.mjs','tests/records.test.mjs','tests/revision.test.mjs'],{stdio:'inherit',timeout:60000});
